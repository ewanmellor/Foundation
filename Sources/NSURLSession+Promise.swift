...
extension URLSession {
     ...
     */
    public func dataTask(_: PMKNamespacer, with convertible: URLRequestConvertible) -> Promise<(Data, URLResponse)> {
        return Promise { dataTask(with: convertible.pmkRequest, completionHandler: adapter($0)).resume() }
    }

    public func uploadTask(_: PMKNamespacer, with convertible: URLRequestConvertible, from data: Data) -> Promise<(Data, URLResponse)> {
        return Promise { uploadTask(with: convertible.pmkRequest, from: data, completionHandler: adapter($0)).resume() }
    }

    public func uploadTask(_: PMKNamespacer, with convertible: URLRequestConvertible, fromFile file: URL) -> Promise<(Data, URLResponse)> {
    ...
    public func downloadTask(_: PMKNamespacer, with convertible: URLRequestConvertible, to saveLocation: URL) -> Promise<(saveLocation: URL, response: URLResponse)> {
        ...
        }.map(on: QoS.background) { tmp, rsp in
            try FileManager.default.moveItem(at: tmp, to: saveLocation)
            return (saveLocation, rsp)
        }
    }
}
